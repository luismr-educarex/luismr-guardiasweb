...
<?php
 ...
 require_once 'HorasSemanasDocente.php';
 ...
 $sql_recuperar_ausencias="SELECT horario,dia,hora,grupo,aula,idProfesor,nombre,tarea,observaciones FROM guardia g INNER JOIN docente d ON g.idProfesor=d.id WHERE d.id=".$id_docente.' AND g.semana='.$semana; 
...
$codigo_html2 = mostrarHorariosDocentes2(obtenerHorario2($datosHoras),$id_docente,$docente);
...
function obtenerHorario($lista_horas){
     ...
     print_r(recuperaHoras2($lista_horas,1));
...
function recuperaHoras2($lista,$dia){
    ...
    for($num_clase=0;$num_clase<$num_clases;$num_clase++){
        if($lista[$num_clase]['dia']==$dia){
             ...
             $horaDocente->setId($lista[$num_clase]['idhora']);
             $horaDocente->setDia($lista[$num_clase]['dia']);
             $horaDocente->setHora($lista[$num_clase]['hora']);
             $horaDocente->setContenido($lista[$num_clase]['materia']);
         ...
         <div class="col-sm-2 bg-celda-info"> '.$vectorDias[2].' MIERCOLES
          ...
          <div class="col-sm-2 bg-celda-info"> '.$vectorDias[3].' JUEVES
           ...
           for($i=1;$i<7;$i++){
               ...
               $html =$html.''.construirHTML_hora($semana,1,$i,$lunes[$i],$ausencialunes[$i-1],$infolunes[$i-1]);
                $html =$html.''.construirHTML_hora($semana,2,$i,$martes[$i],$ausenciamartes[$i-1],$infomartes[$i-1]);
                $html =$html.''.construirHTML_hora($semana,3,$i,$miercoles[$i],$ausenciamiercoles[$i-1],$infomiercoles[$i-1]);
                $html =$html.''.construirHTML_hora($semana,4,$i,$jueves[$i],$ausenciajueves[$i-1],$infojueves[$i-1]);
                $html =$html.''.construirHTML_hora($semana,5,$i,$viernes[$i],$ausenciaviernes[$i-1],$infoviernes[$i-1]);
               $html =$html.'</div>';
           }
         ...
         <div class="col-sm-2 bg-celda-info"> '.$vectorDias[2].' MIERCOLES
          ...
          <div class="col-sm-2 bg-celda-info"> '.$vectorDias[3].' JUEVES
           ...
           for($i=1;$i<7;$i++){
             ...
             $html =$html.'<div class="row">';
             $html =$html.'<div class="col-sm-2 bg-celda-info celdaHora">'.$i.'</div>';
             $html =$html.''.construirHTML_hora2($semana,1,$i,$lunes[$i]);
             $html =$html.''.construirHTML_hora2($semana,2,$i,$martes[$i]);
             $html =$html.''.construirHTML_hora2($semana,3,$i,$miercoles[$i]);
             $html =$html.''.construirHTML_hora2($semana,4,$i,$jueves[$i]);
             $html =$html.''.construirHTML_hora2($semana,5,$i,$viernes[$i]);
             $html =$html.'</div>';
           }
...
function construirHTML_hora($semana,$dia,$hora,$contenido_hora,$contenido_ausencia,$info){
...
function construirHTML_hora2($semana,$dia,$hora,$info){
