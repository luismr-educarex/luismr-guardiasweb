...
function imprimirFila($fecha,$semana,$dia,$hora,$listaAusencias){    
    ...
    if($numeroProfesoresAusentes==0){ //no hay ausencias
         ...
         $html =$html.' <tr class="fila2">';
          $html =$html.' <td class="fila2 numeroHora">'.$hora.'</td>';
          $html =$html.' <td class="celda2final sinAusencias campoDocente" colspan="6">SIN AUSENCIAS</td>';
         $html =$html.'</tr>';
    }
        ...
        for($num_ausencia=0;$num_ausencia<$num_ausencias;$num_ausencia++){
           ...
           {
              $html =$html.' <td rowspan="'.$numeroProfesoresAusentes.'" class="fila2 numeroHora">'.$hora.'<p><a class="botonListaGuardias" href="listarGuardiasHechas.php?semana='.$semana.'&fecha='.$fecha.'&dia='.$dia.'&hora='.$hora.'"><i class="glyphicon glyphicon-equalizer" style="font-size:36px"></i></a></p></td>';
           }  
            ...
            $html =$html.'<td class="celda2final campoDatosLargo">'.mostrarDocentesEnGuardia($ausencias[$num_ausencia]->getDia(),$hora,$ausencias[$num_ausencia]->getId(),$ausencias[$num_ausencia]->getIdProfGuardia()).'</td>';
            $html =$html.'</tr>';
